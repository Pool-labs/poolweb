...
- **Screens**: `app/admin/overview` (metrics #80 + funnels #81), `app/admin/users` + `[id]` (list/search + suspend/restore + feature-flag toggle; a **disabled** "Impersonate (coming soon)" seam for the deferred #84), `app/admin/pools` + `[id]` (list + suspend/restore + read-only ledger #82), `app/admin/moderation` + `[id]` (reports queue + review, see above), `app/admin/errors` (Errors/Health, see above). Money is integer cents, rendered via `lib/admin/format.ts`.
