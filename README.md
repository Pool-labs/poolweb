...
- **Screens:** **Stats** (the whole site's data in eight tabs — Growth · Activity · Pools · Money · Engagement · Funnels · Geography · Health — over one range control, with charts, a delta against the previous window on every windowed tile, and an explained empty state on any panel that could not be served), Users (search / city filter / suspend-restore / feature flags), Pools (search / city filter / suspend-restore / read-only ledger), Moderation (reports queue + review), Errors & Health (recent API failures + Sentry issues, with an explicit per-source "why is this empty" status). The legacy Firestore-backed waitlist dashboard and its questionnaire statistics live under the production-only gate (linked as "Waitlist").
