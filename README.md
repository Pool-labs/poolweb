...
- **Screens:** Overview (usage metrics + funnels), Users (search / suspend-restore / feature flags), Pools (search / suspend-restore / read-only ledger), Moderation (reports queue + review), Errors & Health (recent API failures + Sentry issues, with an explicit per-source "why is this empty" status). The legacy Firestore-backed waitlist dashboard/stats live under the same gate (linked as "Waitlist").
