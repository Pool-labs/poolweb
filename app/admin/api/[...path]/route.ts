...
import { applyRefreshedTokens, refreshAdminTokens } from '@/lib/admin/adminSession';
...
async function handle(req: NextRequest, context: RouteContext): Promise<Response> {
  ...
  if (apiRes.status === 401) {
    ...
    const refreshed = refreshToken ? await refreshAdminTokens(env, refreshToken) : null;
    if (!refreshed) {
      return NextResponse.json(
    ...
    applyRefreshedTokens(response, env, refreshed);
    return response;
  }
