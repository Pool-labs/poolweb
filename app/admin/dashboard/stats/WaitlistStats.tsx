...
import { csvRow } from '@/lib/waitlist/csv';
...
/**
 * Waitlist questionnaire statistics. Rendered only behind the production gate
 * in `page.tsx`; data comes from the gated `/admin/api/waitlist` route.
 */
export function WaitlistStats() {
  ...
  const loadUsersAndCalculateStats = async () => {
    try {
      const allUsers = await waitlistApi.list();
      setUsers(allUsers);
      calculateStats(allUsers);
      analyzeTextResponses(allUsers);
      setLoadError(null);
    } catch (error) {
      // A failed read must never render as a list with no answers in it.
  ...
  const calculateStats = (users: WaitlistEntry[]) => {
  ...
  const analyzeTextResponses = (users: WaitlistEntry[]) => {
    const surveyUsers = users.filter(u => u.surveyData && u.hasCompletedSurvey);
    ...
    textFields.forEach(({ key, question }) => {
      ...
      surveyUsers.forEach(user => {
        const value: unknown = user.surveyData?.[key];
  ...
  const exportStatsToCSV = () => {
    // Option values are visitor-written answers: every cell goes through
    // `csvRow`, which escapes quotes and neutralises spreadsheet formulas.
    const rows: string[] = [csvRow(['Question', 'Option', 'Count', 'Percentage'])];
    ...
    Object.entries(stats).forEach(([key, stat]) => {
      Object.entries(stat.options).forEach(([option, count]) => {
        ...
        rows.push(csvRow([stat.question, option, count, `${percentage}%`]));
      });
    });
    ...
    if (Object.keys(textAnalysis).length > 0) {
      rows.push(''); // Empty row
      rows.push(csvRow(['Text Response Analysis']));
      rows.push(csvRow(['Question', 'Total Responses', 'Positive %', 'Neutral %', 'Negative %', 'Top Themes']));
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        {loadError && (
          <div role="alert" className="mb-8 rounded-md border border-destructive/50 p-4 text-sm text-destructive">
